        ...
        <div class="content">
            ...
            <ul>
                ...
                <ul class="additional-services">
                    @php
                    $extrasJson = stripslashes($booking->extras) ?? [];
                    $extras = json_decode($extrasJson, true);
                    @endphp

                    @if (!empty($extras) && is_array($extras))
                    @foreach ($extras as $item => $details)
                    <li>
                ...
                <li><strong>Frequency:</strong>
                    @if ($booking->frequency == 'week')
                    Weekly 15%
                    @elseif ($booking->frequency == 'month')
                    Monthly 5%
                    @elseif ($booking->frequency == 'fortnight')
                    Fortnightly 10%
                    @else
                    One time
                    @endif
            ...
            <ul>
                ...
                <li><strong>Service Address:</strong> {{ $booking->street ?? '' }}, {{ $booking->apt ?? '' }}&nbsp;{{
                    $booking->city ?? '' }}, {{ $booking->postal_code ?? '' }}</li>
            </ul>
